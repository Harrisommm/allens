...
const en = {
  ...
  mayContain: (names: string) => `May contain · ${names}`,
  mayContainBody: 'Not in the ingredients — named in a cross-contact warning on the package.',
  alsoMayContain: (names: string) => `Also may contain · ${names}`,
  advisorySection: 'Allergy advisory',
...
const ko: Copy = {
  ...
  mayContain: (names: string) => `주의 · ${names} 혼입 가능`,
  mayContainBody: '원재료에는 없지만 포장의 교차오염 주의 문구에 표시됨.',
  alsoMayContain: (names: string) => `추가 주의 · ${names} 혼입 가능`,
  advisorySection: '알레르기 주의사항',
  safe: '안전 · 해당 없음',
  notChecked: '확인 안 됨',
...
const ja: Copy = {
  ...
  mayContain: (names: string) => `注意 · ${names} 混入の可能性`,
  mayContainBody: '原材料には記載なし。パッケージのコンタミネーション注意表示にあり。',
  alsoMayContain: (names: string) => `追加注意 · ${names} 混入の可能性`,
  advisorySection: 'アレルギー注意表示',
  safe: '安全 · 該当なし',
  notChecked: '未確認',
