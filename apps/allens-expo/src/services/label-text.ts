...
const SECTION_START = /원재료\s*명?|성분\s*명?|원료\s*명?|ingredients?/i;

/** Any other section on the package — where the ingredient block ends. */
const SECTION_END =
  /내용량|중량|영양\s*성분|유통\s*기한|소비\s*기한|제조\s*원|제조\s*사|제조\s*일|판매\s*원|판매\s*자|수입\s*원|보관\s*방법|반품|교환|고객\s*상담|소비자\s*상담|품목\s*보고\s*번호|식품\s*유형|포장\s*재질|주소|전화|홈페이지|nutrition|net\s*wt|calories|distributed\s*by|manufactured\s*by|best\s*before|expir|storage|customer\s*service/i;

/**
 * Allergen advisories that legally sit *outside* the ingredient list
 * ("알레르기 유발물질: 우유, 대두 함유", "Contains: milk"). These must survive
 * the trim — they are often the only place an allergen is named.
 */
